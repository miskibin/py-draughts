...
def benchmark_py_draughts():
    ...
    return {
        k: {"mean": mean(v), "median": median(v), "stdev": stdev(v) if len(v) > 1 else 0}
...
def generate_comparison_chart(py_draughts_results, pydraughts_results, output_path):
    ...
    ax.set_title(
        "py-draughts vs pydraughts Performance Comparison",
        fontsize=18,
        color="white",
        fontweight="bold",
        pad=20,
    ...
    for i, (bar1, bar2, speedup) in enumerate(zip(bars1, bars2, speedups)):
        ...
        ax.annotate(
            ...
            bbox=dict(boxstyle="round,pad=0.3", facecolor="#238636", edgecolor="none", alpha=0.8),
        )
    ...
    # Legend
    legend = ax.legend(
        loc="upper left", fontsize=12, framealpha=0.9, facecolor="#161b22", edgecolor="#30363d"
    )
...
def main():
    ...
    chart_path = (
        Path(__file__).parent.parent / "docs" / "source" / "_static" / "speed_comparison.png"
    )
    ...
    for op_name, op_key in operations:
        ...
        print(
            f"{op_name:<20} {format_time(py_time):>15} {format_time(pypi_time):>15} {speedup:>20}"
        )
    ...
    for row in markdown_rows:
        print(
            f"| {row['operation']} | {row['py_draughts']} | {row['pydraughts']} | {row['speedup']} |"
        )
